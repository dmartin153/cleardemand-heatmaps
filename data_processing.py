    ...
    for metric in metrics:
        for norm_to in norm_tos:
            for product in df['{}Id'.format(norm_to)].unique():
                ...
                column = 'Norm{}{}'.format(norm_to, metric)
                df.loc[inds, column] = (df.loc[inds, target] - avg) / std
...
def add_key_points(df, strategies=10):
    ...
    for _, row in df.iterrows():
        ...
        strat_profits, strat_revs = efficient_frontier.find_strategy_prof_rev(pot_revs,
                                                                              pot_profs,
                                                                              profit_weights,
                                                                              revenue_weights)
...
def build_isoforest_preds(df):
    ...
    IsoForestPredict_std -- prediction made from an isolation forest
                            on the standard deviations
    IsoForestPredict_var -- prediction made from an isolation forest
                            on the absolute variation
    IsoForestPredict_stdvar -- prediction made from an isolation forest
                            on the standard deviation and absolute variations
    The Following are trained off only the top 256 revenue items:
    IsoForestPredict_std_toprev -- prediction made from an isolation forest
                                    on the standard deviation
    IsoForestPredict_var_toprev -- prediction made from an isolation forest
                                    on the absolute variation
    IsoForestPredict_stdvar_toprev -- prediction made from an isolation forest
                                    on the standard deviation and absolute variation
    ...
    prices = efficient_frontier.find_closest_strat(df, indexes=inds)
    df.loc[inds, 'IsoForestPrice'] = prices.values
    all_prices = efficient_frontier.find_closest_strat(df)
