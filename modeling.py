...
def build_regression_model(df, model=LinearRegression(),
                           features=('NumIdenticalPrices', 'Cost', 'CurPrice'),
                           target='CurRev', rand_state=20):
    '''This function performs a train test split and applies the provided sklearn
    regression model to the dataframe provided.'''
    X, y = feature_cleaning(df, cols=list(features), target=target)
...
def build_new_suggested_prices(df, outliers):
    ...
    for outlier in outliers:
        df.loc[outlier, 'IsoSugPrice'] = df.loc[outlier, 'IsoSugPrice'] - \
                                         df.loc[outlier, 'CurPriceVariation'] / 2

def add_iso_sug_price(df):
    '''runs iso_forest_predict_outliers and build_new_suggested_prices for the
    provided dataframe'''
...
def isoforestpred(df, fit_option, training_option):
    '''returns predicted outliers for the dataframe provided with the fit
